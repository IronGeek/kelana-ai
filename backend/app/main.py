...
def list_trips(
    ...
    data = (
        db.query(Trip)
        .options(defer(Trip.recommendation))
...
async def search_trip(
    ...
    try:
        ...
        if search != "":
            if request.filter is None or (
                request.filter.destination == request.filter.style
            ):
                query = query.filter(
                    or_(
                        Trip.destination.ilike(f"%{search}%"),
                        literal(search).ilike(any_(Trip.travel_style)),
                    )
                )
            else:
        ...
        page = (
            TripSearchPage(index=1, size=10) if request.page is None else request.page
        )
...
def search_conversations(
    request: SearchConversationRequest | None = None,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    try:
        ...
        if not request is None:
            ...
            page = (
                TripSearchPage(index=1, size=10)
